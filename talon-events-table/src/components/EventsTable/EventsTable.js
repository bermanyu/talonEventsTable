...
const EventsTable = () => {
    ...
    const severityColumn  = {
        ...
        renderCell:   (params) => (
            <div>
                {params.row.severity === 'low' &&
                    <span className='badge low'>{params.row.severity.toUpperCase()}</span>}
                {params.row.severity === 'medium' &&
                    <span className='badge medium'>{params.row.severity.toUpperCase()}</span>}
                {params.row.severity === 'high' &&
                    <span className='badge high'>{params.row.severity.toUpperCase()}</span>}
            </div>
        )
    };
    const userColumn      = {
    ...
    const onEventsTypesFilterHandler = (enteredEventsTypesList) => {
        ...
        if(_.isEmpty(enteredEventsTypesList)) {
            ...
            const newFilteredRows = _.filter(mainData, (row) => {
                const isRowTypeInFilterList = _.find(enteredEventsTypesList, (type) => {
...
const loadDataFromServer = async () => {
    let retVal                = [];
    const dataTablePromise    = await fetch("http://localhost:8000/dataTable");
    const handlePromise       = await dataTablePromise.json();
